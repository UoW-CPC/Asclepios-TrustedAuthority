...
bool Crypto::init_mbedtls(void)
{
  ...
  /*  // Test only - Write out the private key in PEM format for exchange with other enclaves.
    res = mbedtls_pk_write_key_pem(
        &m_pk_context, m_private_key, sizeof(m_private_key));

    if (res != 0)
    {
        TRACE_ENCLAVE("mbedtls_pk_write_key_pem failed (%d)\n", res);
        goto exit;
    }
*/
    ret = true;
...
//test only
void Crypto::retrieve_private_key(uint8_t pem_private_key[512])
{
    uint8_t key[512];
    int res = mbedtls_pk_write_key_pem(
        &m_pk_context, key,sizeof(key));
	...
	memcpy(pem_private_key, key, sizeof(key));
    }	
}
...
int Crypto::test_decrypt(
    ...
    size_t* data_size)
{
  //TRACE_ENCLAVE("decryption");
  //write public and private key
  //unsigned char output_buf[16000];
 // mbedtls_pk_write_key_pem( key, output_buf, 16000 );
  //TRACE_ENCLAVE("private key:%s",output_buf);
  //mbedtls_pk_write_pubkey_pem( key, output_buf, 16000 );
  //TRACE_ENCLAVE("public key:%s",output_buf);
