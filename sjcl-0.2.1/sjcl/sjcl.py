...
class SJCL(object):
    ...
    #    print "salt", hex_string(salt)
        if len(salt) != self.salt_size:
            raise Exception("salt should be %d bytes long." % self.salt_size,"salt:",data["salt"],"salt size:",len(salt))
...
#        print "len", len(nonce)
    ...
    def encrypt(self, plaintext, passphrase, salt = "", iv="", mode="ccm", count=10000, dkLen=16):
        ...
        if salt == "" : # generate salt, if not provided
            print("from sjcl: salt is not provided")
            salt = get_random_bytes(self.salt_size)
        else:
            print("from sjcl, salt:",salt)
            print("from sjcl, encoded salt:",base64.b64encode(salt))
